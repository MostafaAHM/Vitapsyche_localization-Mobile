...
class _ServiceDetailsScreenState extends State<ServiceDetailsScreen> {
  ...
  String _generateChatRoomId(DoctorModel doctor) {
    return 'chat_${_userEmail}_doctor_${doctor.id}';
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(localizations.serviceDetails),
      ),
      body: isLoading
          ? const Center(child: CircularProgressIndicator())
          : serviceDetails == null || doctorDetails == null
              ? Center(child: Text(localizations.failedToLoadDetails))
              : SingleChildScrollView(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      // Service Name
                      Text(
                        serviceDetails!['name'] ?? localizations.noName,
                      ...
                      // Service Price
                      Text(
                        '${localizations.price}: ${serviceDetails!['price'] ?? 'N/A'}',
                        style: const TextStyle(
                          fontSize: 18,
                          color: Colors.green,
                          fontWeight: FontWeight.bold,
                      ...
                      // Service Duration
                      Text(
                        '${localizations.duration}: ${serviceDetails!['duration'] ?? 'N/A'}',
                        style: const TextStyle(
                          fontSize: 16,
                      ...
                      if (doctorDetails != null)
                        DoctorListItem(
                          ...
                          onChat: () {
                            if (doctorDetails != null && _userEmail != null) {
                              Navigator.push(
                                context,
                                MaterialPageRoute(
                                  builder: (context) => DoctorChatScreen(
                                    doctor: doctorDetails!,
                                    chatRoomId:
                                        _generateChatRoomId(doctorDetails!),
                                    currentUserId: _userEmail!,
